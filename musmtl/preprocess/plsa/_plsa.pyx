...
def infer_par(
    ...
    """
    """
    _infer_par(row, col, val, doc_topic, term_topic, n_iters,
...
@cython.cdivision(True)
@cython.boundscheck(False)
def _infer_par(integral[:] dt_row, integral[:] dt_col, floating[:] dt_val,
               floating[:, :] doc_topic, floating[:, :] term_topic,
               int n_iters, floating eps=1e-12, int num_threads=0):
    """
    """
    # declare some local variables
    dtype = np.float64 if floating is double else np.float32
    ...
    for i in range(n_iters):
        ...
        with nogil, parallel(num_threads=num_threads):
            try:
                for idx in prange(nnz, schedule='dynamic'):
                    d, t = dt_row[idx], dt_col[idx]
                    s = eps  # sum to be used for the normalization
                    p = 0.
                    for z in range(n_topics):
                        topic_full[idx, z] = doc_topic[d, z] * term_topic[t, z]
                        s += topic_full[idx, z]
                    for z in range(n_topics):
                        topic_full[idx, z] /= s
            finally:
            ...
            try:
                for idx in prange(nnz, schedule='dynamic'):
                    for z in range(n_topics):
                        ...
                        doc_sum[dt_row[idx]] += q
                # Normalize P(topic | doc)
                for d in prange(n_docs, schedule='dynamic'):
                    for z in range(n_topics):
                        doc_topic[d, z] /= doc_sum[d]
                # Normalize P(term | topic)
                for t in prange(n_terms, schedule='dynamic'):
                    for z in range(n_topics):
                        term_topic[t, z] /= term_sum[z]
            finally:
                pass
